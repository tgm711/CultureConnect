...
def product(id):
            ...
            if form.buy.data:
                ...
                if(form.quantity.data > prod.stock):
                    ...
                    if cart != None:
                        ...
                        db.session.commit()
                        print(c)
                    flash('The product is added to your cart!', 'success')
    if form2.validate_on_submit():
        ...
        if review == [] or review == None:
            ...
            db.session.commit()
            flash('Your Customization is added!', 'success')
            form2.content.data = ''
            reviews = Review.query.filter_by(prod_id=id).all()
        else:
            flash('You have already left a Customization request for this product', 'danger')
...
def removeitem(id):
    ...
    p = []
    cost = []
    status = []  # Initialize the status list
    ...
    for i in range(len(c)):
        ...
        p.append(prod.name)
        cost.append(prod.cost * c[i].quantity)
        # Use available or stock attribute (whichever exists)
        status.append(1 if prod.stock > 0 else 0)  # Change based on your model
...
def deletereview(pid):
    ...
    print(review)
    if review is None:
        flash('There is no Customization to delete', 'info')
    else:
        Review.query.filter_by(user_id=current_user.id, prod_id=pid).delete()
        db.session.commit()
        flash('Your Customization was deleted', 'success')
